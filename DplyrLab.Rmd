...
```{r}
starwars %>% group_by(homeworld)%>% summarise(count=n())
```

If you wanted to arrange this in descending order, what would you add to the pipe? 

```{r}
starwars %>% group_by(homeworld)%>% summarise(count=n()) %>% arrange(count,descending=TRUE)
...
```{r}
starwars %>% group_by(eye_color) %>% arrange(mean(height))
```
